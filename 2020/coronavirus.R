...
confirmed_cases_spread %>%
  ...
  mutate(
    colourin = case_when(
      `Country/Region` == "Mainland China" ~ "#003399",
      `Country/Region` == "Iran" ~ "#FF2B4F",
      `Country/Region` == "Italy" ~ "#fcab27",
      `Country/Region` == "South Korea" ~ "#3686d3",
      T ~ "darkgray")
  ) %>% 
  ggplot(aes(x_axis, date_val, color = colourin)) +
  geom_line(size = 1) +
