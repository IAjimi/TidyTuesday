...
vermin_graph %>% filter(grade %in% c("A", "B", "C")) %>%
  ...
  labs(title = "Restaurant Health Grades & Vermin Sightings", 
       ...
       x = "Number of Vermins",  y = "", fill = "Grade",
       caption = "Source: NYC Department of Health, 2018") 
#numbers slightly different from Five Thirty-Eight bc no "other" category

#### Second Graph (Extension): Average Score Per Grade Over Time ####
another_thing <- nyc_restaurants %>% filter(inspection_date >= "2015-01-01" & 
                                              grade %in% c("A", "B", "C")) %>%
  group_by(inspection_date, grade) %>% summarise(score = mean(score))

ggplot(another_thing, aes(inspection_date, score, color = grade)) + geom_point(alpha = 0.15) + 
  geom_smooth(se = FALSE) + 
  scale_y_reverse() + scale_color_manual(values = c("#47af22", "orange", "red")) + 
  labs(title = "C-Rated Restaurants' Scores Getting Worse", 
       subtitle = "Average Score per Restaurant Health Grade, 2015 - 2018",
       x = "",  y = "Score", color = "Grade",
